...
/**
 * @fileoverview Grok (xAI) provider for AI service layer.
 * OpenAI-compatible API format with xAI's endpoint.
 * @module services/ai/providers/grok
 */
...
const log = createLogger('ai:grok');

const BASE_URL = 'https://api.x.ai/v1/chat/completions';

/**
 * Generate a completion from Grok (xAI) API.
 * @param {Object} params
...
async function generate({ messages, model, temperature = 0.7, maxTokens = 2048 }) {
  const apiKey = config.ai.grok.apiKey;
  if (!apiKey) {
    const err = new Error('Grok API key not configured');
    err.code = 'KEY_MISSING';
    throw err;
  }

  const modelName = model || config.ai.grok.model;
  const maxRetries = 3;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      log.debug('Grok response received', { model: modelName, tokens: usage });
      return { text, usage };
    } catch (err) {
      ...
      log.error('Grok generate failed', {
        status,
        attempt,
